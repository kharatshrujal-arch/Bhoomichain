...
export function WalletButton(): React.JSX.Element {
  ...
  return (
    <>
        ...
        {showDropdown && (
          <div className="absolute right-0 mt-2 w-64 bg-surface-lowest border border-outline-variant rounded-lg shadow-card p-4 z-50 animate-fade-in">
            <div className="border-b border-outline-variant pb-3 mb-3">
              ...
              <div className="font-mono text-label text-on-surface select-all break-all mb-2">
                {publicKey}
              </div>
              <button
                onClick={() => {
                ...
                className="w-full bg-primary-fixed text-primary rounded font-semibold text-xs py-1.5 hover:opacity-90 transition-opacity border-none cursor-pointer flex items-center justify-center gap-1"
              >
      ...
      <OnChainProfileModal isOpen={showProfileModal} onClose={() => setShowProfileModal(false)} />
    </>
  );
}
